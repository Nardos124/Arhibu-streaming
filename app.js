...
document.querySelectorAll('.sidebar a').forEach(link => {
  link.addEventListener('click', e => {
    e.preventDefault();
    const genreId = genreMap[e.target.textContent.trim()];
    if (genreId) {
      displayMovies(`${BASE_URL}/discover/movie?with_genres=${genreId}&sort_by=popularity.desc&page=1`);
    }
  });
});
...
searchInput.addEventListener('keypress', e => {
  if (e.key === 'Enter') {
    ...
    if (query) {
      displayMovies(`${BASE_URL}/search/movie?query=${encodeURIComponent(query)}&page=1`);
    }
  }
});

// ---------------- INITIAL POPULAR MOVIES ----------------
displayMovies(`${BASE_URL}/movie/popular?language=en-US&page=1`);
